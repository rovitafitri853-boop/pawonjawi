<x-app-layout>
    {{-- Header dikembalikan ke bentuk asli --}}
    <x-slot name="header">
    ...
    <div class="h-screen flex flex-col bg-gray-100 py-6" x-data="{ activeCategory: 'Semua', search: '', tipe: 'dine_in' }">
        ...
        <main class="flex-1 flex gap-6 px-6 overflow-hidden">
                ...
                <div class="mb-4 grid grid-cols-3 gap-2">
                    ...
                    <div>
                        ...
                        <select id="selectMeja" name="meja_id" :disabled="tipe === 'take_away'" 
                                    onchange="window.location.href='/kasir/transaksi/'+this.value" 
                                    :required="tipe === 'dine_in'"
                                    class="w-full p-2 border rounded text-[11px] font-bold"
                                    :class="tipe === 'dine_in' && !document.getElementById('selectMeja').value ? 'border-red-500' : 'border-gray-200'">
                            <option value="">-</option> 
                            @foreach(\App\Models\Meja::all() as $m)
                                ...
                                <option value="{{ $m->id }}" 
                                    ...
                                    class="{{ $isTerpakai && !$isMejaAktif ? 'bg-gray-200 text-gray-400' : '' }}">
            ...
            {{-- KOLOM KANAN: MENU (Dalam Card Putih) --}}
                ...
                <div class="grid grid-cols-4 gap-4">
                    @foreach($kategoris as $kat)
                        ...
                        <div class="bg-white p-3 rounded-xl border hover:shadow-md transition" x-show="(activeCategory == 'Semua' || activeCategory == '{{ $kat->nama_kategori }}') && '{{ strtolower($menu->nama_menu) }}'.includes(search.toLowerCase())">
                            ...
                            <button onclick="addToOrder('{{ route('kasir.transaksi.store') }}', {{ $menu->id }}, {{ $menu->harga }}, {{ isset($meja) ? $meja->id : 'null' }})" class="w-full bg-gray-900 text-white py-1.5 rounded-lg text-xs font-bold hover:bg-black">+ Tambah</button>
                        </div>
                        @endforeach
                    @endforeach
                </div>
    ...
    <script>
        ...
        function validateSimpan(event) {
            ...
            if (data.tipe === 'dine_in' && (!selectMeja.value || selectMeja.value === "")) {
                Swal.fire({ icon: 'error', title: 'Perhatian!', text: 'Silakan pilih meja terlebih dahulu untuk tipe Dine In.' });
                return false;
            }
            Swal.fire({
        ...
        function confirmHapusPermanen(url) {
            Swal.fire({
                ...
                if (result.isConfirmed) { window.location.href = url + '?action=hapus'; }
            });
        }
        ...
        function addToOrder(url, menu_id, harga, meja_id) {
            fetch(url, { method: 'POST', headers: { 'X-CSRF-TOKEN': '{{ csrf_token() }}', 'Content-Type': 'application/json' }, body: JSON.stringify({ menu_id, harga, jumlah: 1, meja_id }) }).then(() => window.location.reload());
        }
    </script>
</x-app-layout>
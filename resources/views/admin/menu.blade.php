<x-app-layout>
        ...
        <div class="max-w-7xl mx-auto sm:px-6 lg:px-8">
            <div class="bg-white dark:bg-gray-800 shadow-sm sm:rounded-lg overflow-hidden border border-gray-200 dark:border-gray-700">
                ...
                <div class="p-6 flex flex-col md:flex-row justify-between items-center gap-4">
                    <form action="{{ route('admin.menu.index') }}" method="GET" class="flex gap-2 w-full md:w-auto">
                        ...
                        <a href="{{ route('admin.menu.index') }}" class="bg-gray-500 hover:bg-gray-600 text-white p-2.5 rounded-lg transition" title="Reset">
                            <svg xmlns="http://www.w3.org/2000/svg" class="h-5 w-5" fill="none" viewBox="0 0 24 24" stroke="currentColor">
                    ...
                    <table class="w-full text-left border-collapse border border-gray-200 dark:border-gray-700">
                        <thead class="bg-blue-600 text-white uppercase text-xs">
                            <tr>
                                ...
                                <th class="py-4 px-6 border">Nama Menu</th>
                                <th class="py-4 px-6 border">Kategori</th>
                                <th class="py-4 px-6 border">Harga</th>
                        ...
                        <tbody class="divide-y divide-gray-200 dark:divide-gray-700">
                            ...
                            <tr class="hover:bg-blue-50 dark:hover:bg-gray-600 transition">
                                <td class="py-4 px-2 border text-center text-sm">{{ ($menus->currentPage() - 1) * $menus->perPage() + $loop->iteration }}</td>
                                <td class="py-4 px-6 border text-sm">{{ $item->nama_menu }}</td>
                                <td class="py-4 px-6 border text-sm">{{ $item->kategori->nama_kategori }}</td>
                                <td class="py-4 px-6 border text-sm">Rp {{ number_format($item->harga, 0, ',', '.') }}</td>
                                <td class="py-4 px-6 border text-center">
                                    ...
                                    <form action="{{ route('admin.menu.destroy', $item->id) }}" method="POST" class="inline" id="deleteForm{{ $item->id }}">
                                        @csrf @method('DELETE')
                                        <button type="button" onclick="confirmDelete({{ $item->id }}, '{{ $item->nama_menu }}')" 
                            ...
                            <div id="modalEdit{{ $item->id }}" class="hidden fixed inset-0 z-50 flex items-center justify-center bg-black bg-opacity-50 px-4">
                                <div class="bg-white dark:bg-gray-800 p-6 rounded-lg w-full max-w-sm">
                                    <h3 class="font-bold text-lg mb-4 dark:text-white">Edit Menu</h3>
                                    <form action="{{ route('admin.menu.update', $item->id) }}" method="POST" id="formEdit{{ $item->id }}">
                                        @csrf @method('PUT')
                                        <div class="mb-4">
                                            <label class="block text-sm dark:text-gray-300 font-medium">Nama Menu</label>
                                            <input type="text" name="nama_menu" value="{{ $item->nama_menu }}" class="w-full border-gray-300 rounded-lg dark:bg-gray-700" required>
                                        </div>
                                        <div class="mb-4">
                                            <label class="block text-sm dark:text-gray-300 font-medium">Kategori</label>
                                            <select name="kategori_id" class="w-full border-gray-300 rounded-lg dark:bg-gray-700" required>
                                        ...
                                        <div class="mb-4">
                                            <label class="block text-sm dark:text-gray-300 font-medium">Harga</label>
                                            <input type="number" name="harga" value="{{ $item->harga }}" class="w-full border-gray-300 rounded-lg dark:bg-gray-700" required>
                                        </div>
                            ...
                            @empty
                            <tr><td colspan="5" class="text-center py-10 text-gray-500">Data tidak ditemukan.</td></tr>
                            @endforelse
                        </tbody>
                    </table>
                </div>
                <div class="px-6 pb-6">{{ $menus->withQueryString()->links() }}</div>
            </div>
        </div>
    </div>
    ...
    <div id="modalTambah" class="hidden fixed inset-0 z-50 flex items-center justify-center bg-black bg-opacity-50 px-4">
        <div class="bg-white dark:bg-gray-800 p-6 rounded-lg w-full max-w-sm">
            <h3 class="font-bold text-lg mb-4 dark:text-white">Tambah Menu Baru</h3>
            <form action="{{ route('admin.menu.store') }}" method="POST" id="formStore">
                @csrf
                <div class="mb-4">
                    <label class="block text-sm dark:text-gray-300 font-medium">Nama Menu</label>
                    <input type="text" name="nama_menu" placeholder="Contoh: Nasi Goreng" class="w-full border-gray-300 rounded-lg dark:bg-gray-700" required>
                </div>
                <div class="mb-4">
                    <label class="block text-sm dark:text-gray-300 font-medium">Kategori</label>
                    <select name="kategori_id" class="w-full border-gray-300 rounded-lg dark:bg-gray-700" required>
                ...
                <div class="mb-4">
                    <label class="block text-sm dark:text-gray-300 font-medium">Harga</label>
                    <input type="number" name="harga" placeholder="Contoh: 15000" class="w-full border-gray-300 rounded-lg dark:bg-gray-700" required>
                </div>
                <div class="flex justify-end gap-2">
                    <button type="button" onclick="closeModal(this)" class="bg-gray-300 px-4 py-2 rounded-lg">Batal</button>
